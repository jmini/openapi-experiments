...
public class ApiUtil {
   ...
   *
   * @return a function that can be used in executeAs(executeAs(Function&lt;Response, Response&gt;)
   *     method
   */
  ...
  /**
   * Handler that can be used for Operation that returns {@value HttpStatus#SC_OK}, but where the
   * response content can be empty.
   *
  ...
  public static <T> Function<Response, T> expectOkHandler(Class<T> cls) {
    return r -> {
      ensureStatusCode(r, HttpStatus.SC_OK);
      String response = r.body().asString();
   ...
   *
   * @param statusCode HTTP status code, see {@link HttpStatus} enum
   * @return a function that can be used in execute(Function&lt;Response, Object&gt;) method
   */
  private static <T> Function<Response, T> createHandlerForEmptyResponseWithCustomStatusCode(
    ...
    return r -> {
      ...
      final String result = r.getBody().asString();
